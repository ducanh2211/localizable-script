# Tài liệu Legacy Scripts & Prompt Mẫu AI Ban Đầu

Thư mục này chứa 5 file script Python độc lập ban đầu của dự án (`xcstrings_common.py`, `export_all.py`, `export_untranslated.py`, `merge_csv.py`, `verify.py`).

Dưới đây là toàn bộ tài liệu hướng dẫn nguyên bản, các quy tắc xử lý và **2 Prompt mẫu chuyên dụng cho AI Chatbot**.

---

# Công cụ Localization cho iOS `.xcstrings` (Legacy)
...
```bash
python3 legacy/export_untranslated.py Localizable.xcstrings vi todo.csv
```
...
> ```bash
> python3 legacy/export_all.py Localizable.xcstrings vi all.csv
> ```
...
```bash
python3 legacy/merge_csv.py Localizable.xcstrings todo_translated.csv --lang vi
```
...
```bash
python3 legacy/verify.py Localizable.xcstrings Localizable.merged.xcstrings vi
```
...
```bash
python3 legacy/export_untranslated.py Localizable.xcstrings todo_wide.csv --languages vi,ja,ko
```
...
- Mỗi ngôn ngữ đích có đúng 1 cột `{lang}_target` (**chỗ điền dịch**).
...
```bash
python3 legacy/merge_csv.py Localizable.xcstrings todo_wide_translated.csv
```
...
```bash
python3 legacy/merge_csv.py Localizable.xcstrings todo_wide_translated.csv --dry-run
```

### Bước 4 — Verify nhiều ngôn ngữ cùng lúc

```bash
python3 legacy/verify.py Localizable.xcstrings Localizable.merged.xcstrings vi,ja,ko
```

### Bước 5 — Đưa vào Xcode

Đổi tên `Localizable.merged.xcstrings` thành `Localizable.xcstrings` và mở lại trong Xcode.
